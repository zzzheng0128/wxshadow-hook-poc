...
TOKEN=${RAW_EXIT_HOOK_PRECLEAR_SPLIT_TOKEN:-0x729250}
PROBE_PROC_MAPS=${RAW_EXIT_HOOK_PRECLEAR_SPLIT_PROC_MAPS:-0}
EVIDENCE_DIR="$ROOT/build/evidence"
EVIDENCE="$EVIDENCE_DIR/raw-exit-hook-preclear-hold-split-$(date +%Y%m%d-%H%M%S).log"
MODULE_LOADED=0
SESSION_OPEN=0
HOLD_ACTIVE=0
...
fail() {
  printf '%s\n' "raw exit-hook preclear hold split failure: $*" >&2
  exit 1
}
...
probe_adb_shell_true() {
  ...
  printf 'phase=d4_r3a_shell_true shell_rc=%s output="%s"\n' \
    "$rc" "$output" >> "$EVIDENCE"
...
probe_getprop() {
  ...
  set -e
  output=$(printf '%s' "$output" | LC_ALL=C tr -d '\r')
  printf 'phase=d4_r3a_getprop shell_rc=%s sys.boot_completed="%s"\n' \
    "$rc" "$output" >> "$EVIDENCE"
...
probe_lab_proc_maps() {
  ...
  rc_pid=$?
  set -e
  printf 'phase=d4_r3a_proc_maps_pid pidof_rc=%s lab_pid="%s"\n' \
    "$rc_pid" "$pid" >> "$EVIDENCE"
  [ "$rc_pid" -eq 0 ] && [ -n "$pid" ] ||
  ...
  set -e
  output=$(printf '%s' "$output" | LC_ALL=C tr -d '\r')
  printf 'phase=d4_r3a_proc_maps shell_rc=%s output="%s"\n' \
    "$rc" "$output" >> "$EVIDENCE"
...
cleanup() {
  ...
  if [ "$status" -ne 0 ] && [ "$HOLD_ACTIVE" -eq 1 ]; then
    printf '%s\n' \
      "raw exit-hook preclear hold split preserving module: active hold state remains and cleanup reentry is forbidden" >&2
    exit "$status"
  fi
  ...
  if [ "$MODULE_LOADED" -eq 1 ] && [ "$status" -ne 0 ]; then
    ...
    case "$CLEANUP_STATUS" in
      ...
      *)
        printf '%s\n' \
          "raw exit-hook preclear hold split cleanup did not confirm empty state; preserving module" >&2
        ;;
    esac
  fi
  exit "$status"
...
[ -n "$BOOT_START" ] || fail "empty boot id before d4_r3a setup"
...
printf 'phase=d4_r3a_hold_established result=pass boot_id_reader=not_used raw_slots=2 page_records=2\n' >> "$EVIDENCE"
...
printf 'phase=d4_r3a_hold_quiet_no_boot_reader result=pass boot_id_reader=not_used\n' >> "$EVIDENCE"

probe_adb_shell_true
probe_getprop
probe_lab_proc_maps
require_boot_stable_after_reader d4_r3a_boot_id_reader "$BOOT_START"
printf 'phase=d4_r3a_boot_id_reader result=pass boot_stable=1\n' >> "$EVIDENCE"
...
printf 'phase=d4_r3a_explicit_cleanup result=pass raw_slots=0 page_records=0\n' >> "$EVIDENCE"
...
[ "$BOOT_START" = "$BOOT_END" ] ||
  fail "boot id changed across preclear split: $BOOT_START -> $BOOT_END"
printf 'raw_exit_hook_preclear_hold_split=pass phases=5 boot_reader=pass proc_maps=%s warn_after=%s boot_end=%s final_modules=empty result=pass\n' \
