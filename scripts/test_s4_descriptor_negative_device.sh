...
TOKEN=${S4_DESCRIPTOR_NEGATIVE_TOKEN:-0x540204}
EVIDENCE_DIR="$ROOT/build/evidence"
EVIDENCE="$EVIDENCE_DIR/s4-descriptor-negative-$(date +%Y%m%d-%H%M%S).log"
MODULE_LOADED=0
SESSION_OPEN=0
...
fail() {
  printf '%s\n' "S4 descriptor negative failure: $*" >&2
  exit 1
}
...
cleanup() {
  ...
  if [ "$SESSION_OPEN" -eq 1 ]; then
    ...
    case "$CLEANUP_STATUS" in
      ...
      *)
        printf '%s\n' "S4 descriptor negative cleanup did not confirm empty slots; preserving module" >&2
        cleanup_ok=0
        ;;
    esac
  ...
  if [ "$MODULE_LOADED" -eq 1 ] && [ "$cleanup_ok" -eq 1 ]; then
    if prepare_worker_shutdown >/dev/null 2>&1; then
      ...
      printf '%s\n' "S4 descriptor negative cleanup could not stop workers; preserving module" >&2
    fi
  elif [ "$MODULE_LOADED" -eq 1 ]; then
    printf '%s\n' "S4 descriptor negative cleanup incomplete; module intentionally left resident" >&2
  fi
  exit "$status"
}
...
S4_OUTPUT=$(run_app_command "s4 descriptor negative $TOKEN")
printf '%s\n' "$S4_OUTPUT" >> "$EVIDENCE"
require_contains "$S4_OUTPUT" 's4 mode=descriptor-negative failures=0'
...
require_contains "$S4_OUTPUT" 'probe="s4_descriptor_negative_observed'
require_contains "$S4_OUTPUT" 'baseline_brk_matches=2'
require_contains "$S4_OUTPUT" 'baseline_step_matches=2'
require_contains "$S4_OUTPUT" 'reject_checks=11'
require_contains "$S4_OUTPUT" 'state_intact=1'
require_contains "$S4_OUTPUT" 'bad_brk_offset_rejected=1'
require_contains "$S4_OUTPUT" 'bad_step_offset_rejected=1'
require_contains "$S4_OUTPUT" 'stale_brk_generation_rejected=1'
require_contains "$S4_OUTPUT" 'stale_step_generation_rejected=1'
require_contains "$S4_OUTPUT" 'wrong_brk_slot_rejected=1'
require_contains "$S4_OUTPUT" 'wrong_step_slot_rejected=1'
require_contains "$S4_OUTPUT" 'bad_register_index_rejected=1'
require_contains "$S4_OUTPUT" 'bad_register_value_rejected=1'
require_contains "$S4_OUTPUT" 'cross_slot_brk_rejected=1'
require_contains "$S4_OUTPUT" 'cross_slot_step_rejected=1'
require_contains "$S4_OUTPUT" 'wrong_step_tid_rejected=1'
require_contains "$S4_OUTPUT" 'brk_events=0'
require_contains "$S4_OUTPUT" 'step_events=0'
require_contains "$S4_OUTPUT" 'pte_begin_events=0'
require_contains "$S4_OUTPUT" 'pte_finish_events=0'
...
[ "$BRK_EVENT_COUNT" -eq 0 ] || fail "expected zero S4 BRK events, got $BRK_EVENT_COUNT"
[ "$STEP_EVENT_COUNT" -eq 0 ] || fail "expected zero S4 step events, got $STEP_EVENT_COUNT"
...
printf 's4_descriptor_negative=pass warn_after=%s final_modules=empty result=pass\n' \
