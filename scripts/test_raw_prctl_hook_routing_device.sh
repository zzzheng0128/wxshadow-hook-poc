...
TOKEN=${RAW_PRCTL_HOOK_ROUTING_TOKEN:-0x729408}
EVIDENCE_DIR="$ROOT/build/evidence"
EVIDENCE="$EVIDENCE_DIR/raw-prctl-hook-routing-$(date +%Y%m%d-%H%M%S).log"
MODULE_LOADED=0
SESSION_OPEN=0
...
fail() {
  printf '%s\n' "raw prctl-hook-routing failure: $*" >&2
  exit 1
}
...
cleanup() {
  ...
  if [ "$SESSION_OPEN" -eq 1 ]; then
    ...
    case "$CLEANUP_STATUS" in
      *"raw_slots=0"*) ;;
      *)
        printf '%s\n' "raw prctl-hook-routing cleanup did not confirm empty raw slots; preserving module" >&2
        cleanup_ok=0
        ;;
    esac
  ...
  if [ "$MODULE_LOADED" -eq 1 ] && [ "$cleanup_ok" -eq 1 ]; then
    if prepare_worker_shutdown >/dev/null 2>&1; then
      ...
      printf '%s\n' "raw prctl-hook-routing cleanup could not stop workers; preserving module" >&2
    fi
  elif [ "$MODULE_LOADED" -eq 1 ]; then
    printf '%s\n' "raw prctl-hook-routing cleanup incomplete; module intentionally left resident" >&2
  fi
  exit "$status"
}
...
RAW_OUTPUT=$(run_app_command "raw prctl hook routing run $TOKEN")
printf '%s\n' "$RAW_OUTPUT" >> "$EVIDENCE"
require_contains "$RAW_OUTPUT" 'raw mode=prctl-routing failures=0'
require_contains "$RAW_OUTPUT" 'trigger=prctl_magic'
require_contains "$RAW_OUTPUT" 'route=selected_slot,address'
...
require_contains "$RAW_OUTPUT" 'patch_values=77/88'
require_contains "$RAW_OUTPUT" 'after_patch0=77/99'
require_contains "$RAW_OUTPUT" 'after_release=99/99'
require_contains "$RAW_OUTPUT" 'final=42/42'
require_contains "$RAW_OUTPUT" 'route_slot0_events=1 route_slot1_events=1'
require_contains "$RAW_OUTPUT" 'read_status=1/1'
require_contains "$RAW_OUTPUT" 'patch_isolation=1'
require_contains "$RAW_OUTPUT" 'release_isolation=1'
require_contains "$RAW_OUTPUT" 'patch_events=1/1'
require_contains "$RAW_OUTPUT" 'release_events=1/1'
...
require_contains "$RAW_OUTPUT" 'slot0_capacity_active=1024'
require_contains "$RAW_OUTPUT" 'slot1_capacity_active=0'
...
printf 'raw_prctl_hook_routing=pass route=selected_slot,address page_record_routed=1 warn_after=%s final_modules=empty result=pass\n' \
