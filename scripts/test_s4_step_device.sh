...
TOKEN=${S4_STEP_TOKEN:-0x540101}
EVIDENCE_DIR="$ROOT/build/evidence"
EVIDENCE="$EVIDENCE_DIR/s4-step-$(date +%Y%m%d-%H%M%S).log"
MODULE_LOADED=0
SESSION_OPEN=0
...
cleanup() {
  ...
  if [ "$SESSION_OPEN" -eq 1 ]; then
    ...
    case "$CLEANUP_STATUS" in
      *"s4_slots=0"*) ;;
      *)
        printf '%s\n' "S4 step cleanup did not confirm an empty S4 slot; preserving module" >&2
        cleanup_ok=0
        ;;
    esac
  ...
  if [ "$MODULE_LOADED" -eq 1 ] && [ "$cleanup_ok" -eq 1 ]; then
    if prepare_worker_shutdown >/dev/null 2>&1; then
      ...
      printf '%s\n' "S4 step cleanup could not stop workers; preserving module" >&2
    fi
  elif [ "$MODULE_LOADED" -eq 1 ]; then
    printf '%s\n' "S4 step cleanup incomplete; module intentionally left resident" >&2
  fi
  exit "$status"
}
...
S4_OUTPUT=$(run_app_command "s4 step $TOKEN")
printf '%s\n' "$S4_OUTPUT" >> "$EVIDENCE"
require_contains "$S4_OUTPUT" 's4 mode=step-only failures=0'
require_contains "$S4_OUTPUT" 'value=42'
require_contains "$S4_OUTPUT" 'brk_traps=0'
require_contains "$S4_OUTPUT" 'step_traps=0'
require_contains "$S4_OUTPUT" 'arm="s4_step_ready'
...
require_contains "$S4_OUTPUT" 'brk_skip_origin=1'
require_contains "$S4_OUTPUT" 'step_skip_origin=1'
require_contains "$S4_OUTPUT" 'single_step=1'
...
printf 's4_step=pass warn_after=%s final_modules=empty result=pass\n' \
