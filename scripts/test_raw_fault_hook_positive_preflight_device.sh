...
TOKEN=${RAW_TOKEN:-0x729421}
EVIDENCE_DIR="$ROOT/build/evidence"
EVIDENCE="$EVIDENCE_DIR/raw-fault-hook-positive-preflight-$(date +%Y%m%d-%H%M%S).log"
MODULE_LOADED=0
SESSION_OPEN=0
...
fail() {
  printf '%s\n' "raw fault hook positive-preflight failure: $*" >&2
  exit 1
}
...
cleanup() {
  ...
  if [ "$SESSION_OPEN" -eq 1 ]; then
    ...
    case "$CLEANUP_STATUS" in
      *"raw_slots=0"*) ;;
      *)
        printf '%s\n' "raw fault hook positive-preflight cleanup did not confirm empty raw slots; preserving module" >&2
        cleanup_ok=0
        ;;
    esac
  ...
  if [ "$MODULE_LOADED" -eq 1 ] && [ "$cleanup_ok" -eq 1 ]; then
    if prepare_worker_shutdown >/dev/null 2>&1; then
      ...
      printf '%s\n' "raw fault hook positive-preflight cleanup could not stop workers; preserving module" >&2
    fi
  elif [ "$MODULE_LOADED" -eq 1 ]; then
    printf '%s\n' "raw fault hook positive-preflight cleanup incomplete; module intentionally left resident" >&2
  fi
  exit "$status"
}
...
RAW_OUTPUT=$(run_app_command "raw fault hook positive preflight run $TOKEN")
printf '%s\n' "$RAW_OUTPUT" >> "$EVIDENCE"
require_contains "$RAW_OUTPUT" 'raw mode=fault-hook-positive-preflight'
require_contains "$RAW_OUTPUT" 'fault_route=handle_mm_fault'
require_contains "$RAW_OUTPUT" 'source=file_backed_rx'
require_contains "$RAW_OUTPUT" 'pte_af_clear=1'
...
require_contains "$RAW_OUTPUT" 'af_reads=52800c60/52800c60'
...
require_contains "$RAW_OUTPUT" 'af_signals=0/0'
require_contains "$RAW_OUTPUT" 'raw_slot_fault_hook_ready slot=0'
require_contains "$RAW_OUTPUT" 'raw_slot_fault_hook_ready slot=1'
require_contains "$RAW_OUTPUT" 'raw_slot_fault_af_cleared slot=0'
require_contains "$RAW_OUTPUT" 'raw_slot_fault_af_cleared slot=1'
require_contains "$RAW_OUTPUT" 'status0_after_slot0="raw_slot_fault_hook_status slot=0'
require_contains "$RAW_OUTPUT" 'status1_after_slot0="raw_slot_fault_hook_status slot=1'
require_contains "$RAW_OUTPUT" 'status1_after_slot1="raw_slot_fault_hook_status slot=1'
require_contains "$RAW_OUTPUT" 'status0_after_slot1="raw_slot_fault_hook_status slot=0'

case "$RAW_OUTPUT" in
  *"failures=0"*"positive_route=1"*"page_record_routed=1"*)
    PREFLIGHT_OUTCOME=positive
    ...
    ;;
  *"failures=1"*"positive_route=0"*"page_record_routed=0"*)
    ...
    ;;
  *)
    fail "raw positive preflight output was neither positive nor classified blocked"
    ;;
esac
...
case "$PREFLIGHT_OUTCOME" in
  positive)
    [ "$FAULT_HOOK_HIT_COUNT" -eq 2 ] ||
      fail "expected two raw fault hook hit events, got $FAULT_HOOK_HIT_COUNT"
    ;;
  blocked)
    [ "$FAULT_HOOK_HIT_COUNT" -eq 0 ] ||
      fail "expected zero raw fault hook hit events for blocked preflight, got $FAULT_HOOK_HIT_COUNT"
    ;;
esac
...
case "$PREFLIGHT_OUTCOME" in
  positive)
    printf 'raw_fault_hook_positive_preflight=pass fault_route=handle_mm_fault positive_route=1 warn_after=%s final_modules=empty result=pass\n' \
      "$WARN_AFTER" | tee -a "$EVIDENCE"
    ;;
  blocked)
    printf 'raw_fault_hook_positive_preflight=blocked classification=handle_mm_fault_positive_blocked fault_route=handle_mm_fault source=file_backed_rx pte_af_clear=1 positive_route=0 warn_after=%s final_modules=empty result=pass\n' \
      "$WARN_AFTER" | tee -a "$EVIDENCE"
    ;;
esac
printf '%s\n' "$EVIDENCE"
