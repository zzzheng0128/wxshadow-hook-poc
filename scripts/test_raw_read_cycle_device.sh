...
TOKEN=${RAW_READ_CYCLE_TOKEN:-0x729401}
EVIDENCE_DIR="$ROOT/build/evidence"
EVIDENCE="$EVIDENCE_DIR/raw-read-cycle-$(date +%Y%m%d-%H%M%S).log"
MODULE_LOADED=0
SESSION_OPEN=0
...
cleanup() {
  ...
  if [ "$SESSION_OPEN" -eq 1 ]; then
    ...
    case "$CLEANUP_STATUS" in
      *"raw_slots=0"*) ;;
      *)
        printf '%s\n' "raw read-cycle cleanup did not confirm an empty raw slot; preserving module" >&2
        cleanup_ok=0
        ;;
    esac
  ...
  if [ "$MODULE_LOADED" -eq 1 ] && [ "$cleanup_ok" -eq 1 ]; then
    if prepare_worker_shutdown >/dev/null 2>&1; then
      ...
      printf '%s\n' "raw read-cycle cleanup could not stop workers; preserving module" >&2
    fi
  elif [ "$MODULE_LOADED" -eq 1 ]; then
    printf '%s\n' "raw read-cycle cleanup incomplete; module intentionally left resident" >&2
  fi
  exit "$status"
}
...
RAW_OUTPUT=$(run_app_command "raw read cycle run $TOKEN")
require_contains "$RAW_OUTPUT" 'raw mode=read-cycle failures=0'
require_contains "$RAW_OUTPUT" 'trigger=supercall'
...
require_contains "$RAW_OUTPUT" 'active_kind=original_read'
...
require_contains "$RAW_OUTPUT" 'read_cycle_pte_switch=1'
require_contains "$RAW_OUTPUT" 'read_cycle_data_fault=absent'
...
BEGIN_COUNT=$(printf '%s\n' "$EVENT_OUTPUT" | grep -c 'op=36 result=0' || true)
...
[ "$BEGIN_COUNT" -eq 1 ] || fail "expected one raw read-cycle begin event, got $BEGIN_COUNT"
...
printf 'raw_read_cycle=pass warn_after=%s final_modules=empty result=pass\n' \
