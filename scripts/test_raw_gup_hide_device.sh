...
TOKEN=${RAW_GUP_TOKEN:-0x729101}
EVIDENCE_DIR="$ROOT/build/evidence"
EVIDENCE="$EVIDENCE_DIR/raw-gup-hide-$(date +%Y%m%d-%H%M%S).log"
MODULE_LOADED=0
SESSION_OPEN=0
...
cleanup() {
  ...
  if [ "$SESSION_OPEN" -eq 1 ]; then
    ...
    case "$CLEANUP_STATUS" in
      *"raw_slots=0"*) ;;
      *)
        printf '%s\n' "raw gup-hide cleanup did not confirm an empty raw slot; preserving module" >&2
        cleanup_ok=0
        ;;
    esac
  ...
  if [ "$MODULE_LOADED" -eq 1 ] && [ "$cleanup_ok" -eq 1 ]; then
    if prepare_worker_shutdown >/dev/null 2>&1; then
      ...
      printf '%s\n' "raw gup-hide cleanup could not stop workers; preserving module" >&2
    fi
  elif [ "$MODULE_LOADED" -eq 1 ]; then
    printf '%s\n' "raw gup-hide cleanup incomplete; module intentionally left resident" >&2
  fi
  exit "$status"
}
...
RAW_OUTPUT=$(run_app_command "raw gup run $TOKEN")
require_contains "$RAW_OUTPUT" 'raw mode=gup-hide failures=0'
require_contains "$RAW_OUTPUT" 'normal_value=42'
require_contains "$RAW_OUTPUT" 'shadow_value=99'
require_contains "$RAW_OUTPUT" 'hide_value=42'
require_contains "$RAW_OUTPUT" 'shadow_after_finish=99'
require_contains "$RAW_OUTPUT" 'restored_value=42'
require_contains "$RAW_OUTPUT" 'gup_hide_active=1'
require_contains "$RAW_OUTPUT" 'gup_hide_active=0'
require_contains "$RAW_OUTPUT" 'gup_begin_events=1'
require_contains "$RAW_OUTPUT" 'gup_finish_events=1'
require_contains "$RAW_OUTPUT" 'original_view_after_shadow=proven'
require_contains "$RAW_OUTPUT" 'gup_hide_primitive=proven'
...
GUP_BEGIN_COUNT=$(printf '%s\n' "$EVENT_OUTPUT" | grep -c 'op=27 result=0' || true)
GUP_FINISH_COUNT=$(printf '%s\n' "$EVENT_OUTPUT" | grep -c 'op=28 result=0' || true)
[ "$ACTIVATE_COUNT" -eq 1 ] || fail "expected one raw activation event, got $ACTIVATE_COUNT"
[ "$GUP_BEGIN_COUNT" -eq 1 ] || fail "expected one raw gup begin event, got $GUP_BEGIN_COUNT"
[ "$GUP_FINISH_COUNT" -eq 1 ] || fail "expected one raw gup finish event, got $GUP_FINISH_COUNT"
...
printf 'raw_gup_hide=pass warn_after=%s final_modules=empty result=pass\n' \
