...
TOKEN=${RAW_ABORT_READ_CYCLE_TOKEN:-0x729408}
EVIDENCE_DIR="$ROOT/build/evidence"
EVIDENCE="$EVIDENCE_DIR/raw-abort-read-cycle-$(date +%Y%m%d-%H%M%S).log"
MODULE_LOADED=0
SESSION_OPEN=0
...
fail() {
  printf '%s\n' "raw abort-read-cycle failure: $*" >&2
  exit 1
}
...
cleanup() {
  ...
  if [ "$SESSION_OPEN" -eq 1 ]; then
    run_app_command "raw abort read cycle clear $TOKEN" >/dev/null 2>&1 || true
    run_app_command "raw clear $TOKEN" >/dev/null 2>&1 || true
    CLEANUP_STATUS=$(run_app_command status 2>&1 || true)
    case "$CLEANUP_STATUS" in
      *"raw_slots=0"*) ;;
      *)
        printf '%s\n' "raw abort-read-cycle cleanup did not confirm an empty raw slot; preserving module" >&2
        cleanup_ok=0
        ;;
    esac
  ...
  if [ "$MODULE_LOADED" -eq 1 ] && [ "$cleanup_ok" -eq 1 ]; then
    if prepare_worker_shutdown >/dev/null 2>&1; then
      ...
      printf '%s\n' "raw abort-read-cycle cleanup could not stop workers; preserving module" >&2
    fi
  elif [ "$MODULE_LOADED" -eq 1 ]; then
    printf '%s\n' "raw abort-read-cycle cleanup incomplete; module intentionally left resident" >&2
  fi
  exit "$status"
}
...
RAW_OUTPUT=$(run_app_command "raw abort read cycle run $TOKEN")
printf '%s\n' "$RAW_OUTPUT" >> "$EVIDENCE"
require_contains "$RAW_OUTPUT" 'raw mode=abort-read-cycle failures=0'
require_contains "$RAW_OUTPUT" 'trigger=do_mem_abort'
...
require_contains "$RAW_OUTPUT" 'skip_origin=1'
require_contains "$RAW_OUTPUT" 'observe_only=0'
...
require_contains "$RAW_OUTPUT" 'read_completed=1'
require_contains "$RAW_OUTPUT" 'mprotect_none_rc=0'
require_contains "$RAW_OUTPUT" 'handler_faults=0'
require_contains "$RAW_OUTPUT" 'raw_abort_read_cycle_ready'
require_contains "$RAW_OUTPUT" 'raw_abort_read_cycle_status'
require_contains "$RAW_OUTPUT" 'read_events=1'
require_contains "$RAW_OUTPUT" 'failures=0'
require_contains "$RAW_OUTPUT" 'last_ec=36'
require_contains "$RAW_OUTPUT" 'last_fsc_type=4'
require_contains "$RAW_OUTPUT" 'last_wnr=0'
require_contains "$RAW_OUTPUT" 'translation_fault=1'
require_contains "$RAW_OUTPUT" 'begin_result=0'
require_contains "$RAW_OUTPUT" 'read_cycle_active=1'
require_contains "$RAW_OUTPUT" 'read_cycle_active=0'
...
require_contains "$RAW_OUTPUT" 'abort_read_cycle_events=1'
require_contains "$RAW_OUTPUT" 'read_cycle_data_fault=sync_el0_dabt'
require_contains "$RAW_OUTPUT" 'read_cycle_exec_resume=proven'
require_contains "$RAW_OUTPUT" 'record_state=shadow_active'
...
[ "$BEGIN_COUNT" -eq 1 ] || fail "expected one raw abort read-cycle event, got $BEGIN_COUNT"
[ "$FINISH_COUNT" -eq 1 ] || fail "expected one raw read-cycle finish event, got $FINISH_COUNT"
...
printf 'raw_abort_read_cycle=pass warn_after=%s final_modules=empty result=pass\n' \
