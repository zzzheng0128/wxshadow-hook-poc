...
cleanup() {
  ...
  if [ "$SESSION_OPEN" -eq 1 ]; then
    run_app_command "raw prctl read cycle hook clear $TOKEN" >/dev/null 2>&1 || true
...
printf '%s\n' "$RAW_OUTPUT" >> "$EVIDENCE"
require_contains "$RAW_OUTPUT" 'raw mode=prctl-read-cycle failures=0'
require_contains "$RAW_OUTPUT" 'trigger=prctl_magic'
require_contains "$RAW_OUTPUT" 'option=52304c42'
require_contains "$RAW_OUTPUT" 'operation=1'
...
require_contains "$RAW_OUTPUT" 'raw_prctl_read_cycle_hook_ready'
...
PRCTL_TRIGGER_COUNT=$(printf '%s\n' "$EVENT_OUTPUT" | grep -c 'op=42 result=0' || true)
...
[ "$PRCTL_TRIGGER_COUNT" -eq 1 ] || fail "expected one prctl trigger event, got $PRCTL_TRIGGER_COUNT"
...
PRCTL_TRIGGER_SEQ=$(printf '%s\n' "$EVENT_OUTPUT" |
  sed -n 's/.*seq=\([0-9][0-9]*\).*op=42 result=0.*/\1/p')
...
[ "$PRCTL_REJECT_SEQ" -lt "$PRCTL_TRIGGER_SEQ" ] ||
  fail "rejected prctl event did not precede the admitted trigger"
[ "$PRCTL_TRIGGER_SEQ" -lt "$READ_CYCLE_BEGIN_SEQ" ] ||
  fail "admitted prctl event did not precede read-cycle begin"
...
printf 'raw_prctl_read_cycle=pass warn_after=%s final_modules=empty result=pass\n' \
