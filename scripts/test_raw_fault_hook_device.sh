...
TOKEN=${RAW_FAULT_HOOK_TOKEN:-0x729203}
EVIDENCE_DIR="$ROOT/build/evidence"
EVIDENCE="$EVIDENCE_DIR/raw-fault-hook-$(date +%Y%m%d-%H%M%S).log"
MODULE_LOADED=0
SESSION_OPEN=0
...
cleanup() {
  ...
  if [ "$SESSION_OPEN" -eq 1 ]; then
    ...
    case "$CLEANUP_STATUS" in
      *"raw_slots=0"*) ;;
      *)
        printf '%s\n' "raw fault-hook cleanup did not confirm an empty raw slot; preserving module" >&2
        cleanup_ok=0
        ;;
    esac
  ...
  if [ "$MODULE_LOADED" -eq 1 ] && [ "$cleanup_ok" -eq 1 ]; then
    if prepare_worker_shutdown >/dev/null 2>&1; then
      ...
      printf '%s\n' "raw fault-hook cleanup could not stop workers; preserving module" >&2
    fi
  elif [ "$MODULE_LOADED" -eq 1 ]; then
    printf '%s\n' "raw fault-hook cleanup incomplete; module intentionally left resident" >&2
  fi
  exit "$status"
}
...
RAW_OUTPUT=$(run_app_command "raw fault hook run $TOKEN")
printf '%s\n' "$RAW_OUTPUT" >> "$EVIDENCE"
require_contains "$RAW_OUTPUT" 'raw mode=fault-hook failures=0'
...
require_contains "$RAW_OUTPUT" 'shadow_after_hook_value=99'
...
require_contains "$RAW_OUTPUT" 'fault_hook_installed=1'
require_contains "$RAW_OUTPUT" 'fault_hook_read_events=0'
require_contains "$RAW_OUTPUT" 'fault_hook_write_events=0'
require_contains "$RAW_OUTPUT" 'fault_hook_exec_events=0'
...
[ "$FAULT_HOOK_HIT_COUNT" -eq 0 ] || fail "expected no raw fault hook hit event, got $FAULT_HOOK_HIT_COUNT"
...
printf 'raw_fault_hook=pass warn_after=%s final_modules=empty result=pass\n' \
