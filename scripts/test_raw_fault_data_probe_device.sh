...
TOKEN=${RAW_FAULT_DATA_PROBE_TOKEN:-0x729204}
EVIDENCE_DIR="$ROOT/build/evidence"
EVIDENCE="$EVIDENCE_DIR/raw-fault-data-probe-$(date +%Y%m%d-%H%M%S).log"
MODULE_LOADED=0
SESSION_OPEN=0
...
fail() {
  printf '%s\n' "raw fault data-probe failure: $*" >&2
  exit 1
}
...
cleanup() {
  ...
  if [ "$SESSION_OPEN" -eq 1 ]; then
    run_app_command "raw fault probe clear $TOKEN" >/dev/null 2>&1 || true
    run_app_command "raw fault hook clear $TOKEN" >/dev/null 2>&1 || true
    run_app_command "raw clear $TOKEN" >/dev/null 2>&1 || true
    CLEANUP_STATUS=$(run_app_command status 2>&1 || true)
    case "$CLEANUP_STATUS" in
      *"raw_slots=0"*) ;;
      *)
        printf '%s\n' "raw fault data-probe cleanup did not confirm an empty raw slot; preserving module" >&2
        cleanup_ok=0
        ;;
    esac
  ...
  if [ "$MODULE_LOADED" -eq 1 ] && [ "$cleanup_ok" -eq 1 ]; then
    if prepare_worker_shutdown >/dev/null 2>&1; then
      ...
      printf '%s\n' "raw fault data-probe cleanup could not stop workers; preserving module" >&2
    fi
  elif [ "$MODULE_LOADED" -eq 1 ]; then
    printf '%s\n' "raw fault data-probe cleanup incomplete; module intentionally left resident" >&2
  fi
  exit "$status"
}
...
RAW_OUTPUT=$(run_app_command "raw fault data probe run $TOKEN")
printf '%s\n' "$RAW_OUTPUT" >> "$EVIDENCE"
require_contains "$RAW_OUTPUT" 'raw mode=fault-data-probe failures=0'
require_contains "$RAW_OUTPUT" 'data_fault_source=normal_anon_remote_gup'
require_contains "$RAW_OUTPUT" 'observe_only=1'
require_contains "$RAW_OUTPUT" 'target_mm_scoped=1'
require_contains "$RAW_OUTPUT" 'remote_only=1'
...
require_contains "$RAW_OUTPUT" 'reader_rc=4'
require_contains "$RAW_OUTPUT" 'reader_errno=0'
require_contains "$RAW_OUTPUT" 'child_exit=0'
require_contains "$RAW_OUTPUT" 'ready_read=1'
require_contains "$RAW_OUTPUT" 'start_write=1'
require_contains "$RAW_OUTPUT" 'result_read=12'
require_contains "$RAW_OUTPUT" 'raw_fault_hook_ready'
require_contains "$RAW_OUTPUT" 'raw_fault_probe_ready'
require_contains "$RAW_OUTPUT" 'raw_fault_probe_reader_ready'
require_contains "$RAW_OUTPUT" 'symbol=handle_mm_fault'
require_contains "$RAW_OUTPUT" 'source=normal_anon_remote_gup'
...
require_contains "$RAW_OUTPUT" 'fault_hook_read_events=0'
require_contains "$RAW_OUTPUT" 'fault_probe_armed=1'
require_contains "$RAW_OUTPUT" 'fault_probe_read_events=1'
require_contains "$RAW_OUTPUT" 'fault_probe_write_events=0'
require_contains "$RAW_OUTPUT" 'fault_probe_exec_events=0'
require_contains "$RAW_OUTPUT" 'fault_probe=normal_anon_remote_gup'
require_contains "$RAW_OUTPUT" 'fault_hook=observe_only'
...
[ "$FAULT_HOOK_HIT_COUNT" -eq 1 ] || fail "expected one raw fault data-probe hit event, got $FAULT_HOOK_HIT_COUNT"
...
printf 'raw_fault_data_probe=pass warn_after=%s final_modules=empty result=pass\n' \
