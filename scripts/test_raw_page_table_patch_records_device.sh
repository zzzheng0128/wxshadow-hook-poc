...
TOKEN=${RAW_TOKEN:-0x729031}
EVIDENCE_DIR="$ROOT/build/evidence"
EVIDENCE="$EVIDENCE_DIR/raw-page-table-patch-records-$(date +%Y%m%d-%H%M%S).log"
MODULE_LOADED=0
SESSION_OPEN=0
...
fail() {
  printf '%s\n' "raw page-table patch-records failure: $*" >&2
  exit 1
}
...
cleanup() {
  ...
  if [ "$SESSION_OPEN" -eq 1 ]; then
    ...
    case "$CLEANUP_STATUS" in
      *"raw_slots=0"*) ;;
      *)
        printf '%s\n' "raw page-table patch-records cleanup did not confirm empty raw slots; preserving module" >&2
        cleanup_ok=0
        ;;
    esac
  ...
  if [ "$MODULE_LOADED" -eq 1 ] && [ "$cleanup_ok" -eq 1 ]; then
    if prepare_worker_shutdown >/dev/null 2>&1; then
      ...
      printf '%s\n' "raw page-table patch-records cleanup could not stop workers; preserving module" >&2
    fi
  elif [ "$MODULE_LOADED" -eq 1 ]; then
    printf '%s\n' "raw page-table patch-records cleanup incomplete; module intentionally left resident" >&2
  fi
  exit "$status"
}
...
RAW_OUTPUT=$(run_app_command "raw page table patch records run $TOKEN")
printf '%s\n' "$RAW_OUTPUT" >> "$EVIDENCE"
require_contains "$RAW_OUTPUT" 'raw mode=page-table-patch-records failures=0'
require_contains "$RAW_OUTPUT" 'normal0=42 normal1=42 shadow0=99 shadow1=99'
require_contains "$RAW_OUTPUT" 'slot0_patch_value=77 slot1_patch_value=88'
require_contains "$RAW_OUTPUT" 'after_release0_slot0=99 after_release0_slot1=88'
require_contains "$RAW_OUTPUT" 'slot0_patch_record_slots=1024 slot0_patch_active_count=1024'
require_contains "$RAW_OUTPUT" 'slot0_patch_dirty_bytes=1024'
require_contains "$RAW_OUTPUT" 'slot0_release_slots=1 slot0_release_active=0 slot0_release_dirty=0'
require_contains "$RAW_OUTPUT" 'capacity_fill_records=1024 capacity_fill_failures=0'
require_contains "$RAW_OUTPUT" 'slot0_capacity_overflow_errno=28'
require_contains "$RAW_OUTPUT" 'slot1_patch_record_slots=1 slot1_patch_active_count=1'
require_contains "$RAW_OUTPUT" 'slot1_patch_dirty_bytes=4'
require_contains "$RAW_OUTPUT" 'slot1_after_slot0_capacity_value=88'
require_contains "$RAW_OUTPUT" 'final_original_slot0=42 final_original_slot1=42'
require_contains "$RAW_OUTPUT" 'handler_faults=0'
require_contains "$RAW_OUTPUT" 'raw_slot_patch_status slot=0'
require_contains "$RAW_OUTPUT" 'raw_slot_patch_status slot=1'
require_contains "$RAW_OUTPUT" 'patch_scope=page_slot_ranges'
...
printf 'raw_page_table_patch_records=pass warn_after=%s final_modules=empty result=pass\n' \
