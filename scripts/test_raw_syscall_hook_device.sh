...
TOKEN=${RAW_SYSCALL_HOOK_TOKEN:-0x729205}
EVIDENCE_DIR="$ROOT/build/evidence"
EVIDENCE="$EVIDENCE_DIR/raw-syscall-hook-$(date +%Y%m%d-%H%M%S).log"
MODULE_LOADED=0
SESSION_OPEN=0
...
cleanup() {
  ...
  if [ "$SESSION_OPEN" -eq 1 ]; then
    run_app_command "raw syscall hook clear $TOKEN" >/dev/null 2>&1 || true
    run_app_command "raw clear $TOKEN" >/dev/null 2>&1 || true
    CLEANUP_STATUS=$(run_app_command status 2>&1 || true)
    case "$CLEANUP_STATUS" in
      *"raw_slots=0"*) ;;
      *)
        printf '%s\n' "raw syscall-hook cleanup did not confirm an empty raw slot; preserving module" >&2
        cleanup_ok=0
        ;;
    esac
  ...
  if [ "$MODULE_LOADED" -eq 1 ] && [ "$cleanup_ok" -eq 1 ]; then
    if prepare_worker_shutdown >/dev/null 2>&1; then
      ...
      printf '%s\n' "raw syscall-hook cleanup could not stop workers; preserving module" >&2
    fi
  elif [ "$MODULE_LOADED" -eq 1 ]; then
    printf '%s\n' "raw syscall-hook cleanup incomplete; module intentionally left resident" >&2
  fi
  exit "$status"
}
...
RAW_OUTPUT=$(run_app_command "raw syscall hook run $TOKEN")
printf '%s\n' "$RAW_OUTPUT" >> "$EVIDENCE"
require_contains "$RAW_OUTPUT" 'raw mode=syscall-hook failures=0'
...
require_contains "$RAW_OUTPUT" 'raw_syscall_hook_ready'
...
case "$RAW_OUTPUT" in
  *"hit_events=0"*) fail "syscall hook did not observe getpid trigger" ;;
esac
...
[ "$SYSCALL_HOOK_COUNT" -ge 1 ] || fail "expected at least one raw syscall hook event, got $SYSCALL_HOOK_COUNT"
...
printf 'raw_syscall_hook=pass warn_after=%s final_modules=empty result=pass\n' \
