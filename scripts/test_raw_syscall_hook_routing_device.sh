...
TOKEN=${RAW_SYSCALL_HOOK_ROUTING_TOKEN:-0x729452}
EVIDENCE_DIR="$ROOT/build/evidence"
EVIDENCE="$EVIDENCE_DIR/raw-syscall-hook-routing-$(date +%Y%m%d-%H%M%S).log"
MODULE_LOADED=0
SESSION_OPEN=0
...
fail() {
  printf '%s\n' "raw syscall-hook-routing failure: $*" >&2
  exit 1
}
...
cleanup() {
  ...
  if [ "$SESSION_OPEN" -eq 1 ]; then
    ...
    case "$CLEANUP_STATUS" in
      *"raw_slots=0"*) ;;
      *)
        printf '%s\n' "raw syscall-hook-routing cleanup did not confirm empty raw slots; preserving module" >&2
        cleanup_ok=0
        ;;
    esac
  ...
  if [ "$MODULE_LOADED" -eq 1 ] && [ "$cleanup_ok" -eq 1 ]; then
    if prepare_worker_shutdown >/dev/null 2>&1; then
      ...
      printf '%s\n' "raw syscall-hook-routing cleanup could not stop workers; preserving module" >&2
    fi
  elif [ "$MODULE_LOADED" -eq 1 ]; then
    printf '%s\n' "raw syscall-hook-routing cleanup incomplete; module intentionally left resident" >&2
  fi
  exit "$status"
}
...
RAW_OUTPUT=$(run_app_command "raw syscall read cycle routing run $TOKEN")
printf '%s\n' "$RAW_OUTPUT" >> "$EVIDENCE"
require_contains "$RAW_OUTPUT" 'raw mode=syscall-read-cycle-routing failures=0'
require_contains "$RAW_OUTPUT" 'trigger=syscall_getpid'
require_contains "$RAW_OUTPUT" 'read_cycle=uxn_original_exec_resume'
require_contains "$RAW_OUTPUT" 'route=selected_slot'
require_contains "$RAW_OUTPUT" 'page_record_routed=1'
require_contains "$RAW_OUTPUT" 'target_mm_scoped=1'
require_contains "$RAW_OUTPUT" 'stale_generation_rc=-11'
require_contains "$RAW_OUTPUT" 'normal=42/42 shadow=99/99'
require_contains "$RAW_OUTPUT" 'original_read_word=52800540/52800540'
require_contains "$RAW_OUTPUT" 'resume=99/99'
require_contains "$RAW_OUTPUT" 'shadow_after_resume=52800c60/52800c60'
require_contains "$RAW_OUTPUT" 'final=42/42'
require_contains "$RAW_OUTPUT" 'route_slot0_events=1 route_slot1_events=1'
require_contains "$RAW_OUTPUT" 'activations=1/1 states=3/3'
require_contains "$RAW_OUTPUT" 'hook_ready=1/1 selected=1/1'
require_contains "$RAW_OUTPUT" 'status_slot0=1/1/1'
require_contains "$RAW_OUTPUT" 'status_slot1=1/1/1'
require_contains "$RAW_OUTPUT" 'clear_hit=1/1 clear_finish=1/1 clear_failures=0/0'
...
SYSCALL_HOOK_COUNT=$(printf '%s\n' "$EVENT_OUTPUT" | grep -c 'op=35 result=0' || true)
...
[ "$ACTIVATE_COUNT" -ge 1 ] || fail "expected a visible raw activation event, got $ACTIVATE_COUNT"
[ "$SYSCALL_HOOK_COUNT" -eq 2 ] || fail "expected two raw syscall hook events, got $SYSCALL_HOOK_COUNT"
[ "$SYSCALL_READ_CYCLE_COUNT" -eq 2 ] || fail "expected two syscall read-cycle begin events, got $SYSCALL_READ_CYCLE_COUNT"
[ "$READ_CYCLE_FINISH_COUNT" -eq 2 ] || fail "expected two raw read-cycle finish events, got $READ_CYCLE_FINISH_COUNT"
[ "$CLEAR_COUNT" -eq 2 ] || fail "expected two raw clear events, got $CLEAR_COUNT"
...
printf 'raw_syscall_hook_routing=pass route=syscall_getpid page_record_routed=1 warn_after=%s final_modules=empty result=pass\n' \
