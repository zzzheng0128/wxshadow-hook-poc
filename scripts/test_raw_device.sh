...
TOKEN=${RAW_TOKEN:-0x729001}
EVIDENCE_DIR="$ROOT/build/evidence"
EVIDENCE="$EVIDENCE_DIR/raw-device-$(date +%Y%m%d-%H%M%S).log"
MODULE_LOADED=0
SESSION_OPEN=0
...
cleanup() {
  ...
  if [ "$SESSION_OPEN" -eq 1 ]; then
    ...
    case "$CLEANUP_STATUS" in
      *"raw_slots=0"*) ;;
      *)
        printf '%s\n' "raw cleanup did not confirm an empty raw slot; preserving module" >&2
        cleanup_ok=0
        ;;
    esac
  ...
  if [ "$MODULE_LOADED" -eq 1 ] && [ "$cleanup_ok" -eq 1 ]; then
    if prepare_worker_shutdown >/dev/null 2>&1; then
      ...
      printf '%s\n' "raw cleanup could not stop workers; preserving module" >&2
    fi
  elif [ "$MODULE_LOADED" -eq 1 ]; then
    printf '%s\n' "raw cleanup incomplete; module intentionally left resident" >&2
  fi
  exit "$status"
}
...
RAW_OUTPUT=$(run_app_command "raw run $TOKEN")
printf '%s\n' "$RAW_OUTPUT" >> "$EVIDENCE"
require_contains "$RAW_OUTPUT" 'raw mode=two-pfn failures=0'
require_contains "$RAW_OUTPUT" 'normal_value=42 shadow_value=99 restored_value=42'
require_contains "$RAW_OUTPUT" 'read_view=shadow'
require_contains "$RAW_OUTPUT" 'activations=1 state=3'
require_contains "$RAW_OUTPUT" 'active_kind=shadow_rx'
require_contains "$RAW_OUTPUT" 'read_cycle=absent'
require_contains "$RAW_OUTPUT" 'original_view_after_shadow=absent'
...
printf 'raw_activations=1 warn_after=%s final_modules=empty result=pass\n' \
