...
TOKEN=${S4_RAW_REG_TOKEN:-0x540202}
EVIDENCE_DIR="$ROOT/build/evidence"
EVIDENCE="$EVIDENCE_DIR/s4-raw-reg-$(date +%Y%m%d-%H%M%S).log"
MODULE_LOADED=0
SESSION_OPEN=0
...
cleanup() {
  ...
  if [ "$SESSION_OPEN" -eq 1 ]; then
    ...
    case "$CLEANUP_STATUS" in
      ...
      *)
        printf '%s\n' "S4 raw-reg cleanup did not confirm empty slots; preserving module" >&2
        cleanup_ok=0
        ;;
    esac
  ...
  if [ "$MODULE_LOADED" -eq 1 ] && [ "$cleanup_ok" -eq 1 ]; then
    if prepare_worker_shutdown >/dev/null 2>&1; then
      ...
      printf '%s\n' "S4 raw-reg cleanup could not stop workers; preserving module" >&2
    fi
  elif [ "$MODULE_LOADED" -eq 1 ]; then
    printf '%s\n' "S4 raw-reg cleanup incomplete; module intentionally left resident" >&2
  fi
  exit "$status"
}
...
S4_OUTPUT=$(run_app_command "s4 raw-reg $TOKEN")
printf '%s\n' "$S4_OUTPUT" >> "$EVIDENCE"
require_contains "$S4_OUTPUT" 's4 mode=raw-reg failures=0'
require_contains "$S4_OUTPUT" 'register_edit=1'
require_contains "$S4_OUTPUT" 'register_index=1'
require_contains "$S4_OUTPUT" 'register_value=73'
require_contains "$S4_OUTPUT" 'register_apply=brk_before_single_step'
require_contains "$S4_OUTPUT" 'normal_value=42'
require_contains "$S4_OUTPUT" 'hook_value=73'
...
require_contains "$S4_OUTPUT" 'arm="s4_raw_reg_ready'
...
require_contains "$S4_OUTPUT" 'reg_write_events=1'
require_contains "$S4_OUTPUT" 'state=step_observed'
require_contains "$S4_OUTPUT" 'mode=raw_reg'
...
REG_EVENT_COUNT=$(printf '%s\n' "$EVENT_OUTPUT" | grep -c 'op=41 result=0' || true)
...
[ "$REG_EVENT_COUNT" -eq 1 ] || fail "expected one S4 register event, got $REG_EVENT_COUNT"
...
printf 's4_raw_reg=pass warn_after=%s final_modules=empty result=pass\n' \
