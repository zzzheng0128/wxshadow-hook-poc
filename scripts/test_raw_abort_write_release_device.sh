...
TOKEN=${RAW_ABORT_WRITE_RELEASE_TOKEN:-0x729259}
EVIDENCE_DIR="$ROOT/build/evidence"
EVIDENCE="$EVIDENCE_DIR/raw-abort-write-release-$(date +%Y%m%d-%H%M%S).log"
MODULE_LOADED=0
SESSION_OPEN=0
...
fail() {
  printf '%s\n' "raw abort-write-release failure: $*" >&2
  exit 1
}
...
cleanup() {
  ...
  if [ "$SESSION_OPEN" -eq 1 ]; then
    run_app_command "raw abort write release clear $TOKEN" >/dev/null 2>&1 || true
    run_app_command "raw clear $TOKEN" >/dev/null 2>&1 || true
    CLEANUP_STATUS=$(run_app_command status 2>&1 || true)
    case "$CLEANUP_STATUS" in
      *"raw_slots=0"*) ;;
      *)
        printf '%s\n' "raw abort-write-release cleanup did not confirm an empty raw slot; preserving module" >&2
        cleanup_ok=0
        ;;
    esac
  ...
  if [ "$MODULE_LOADED" -eq 1 ] && [ "$cleanup_ok" -eq 1 ]; then
    if prepare_worker_shutdown >/dev/null 2>&1; then
      ...
      printf '%s\n' "raw abort-write-release cleanup could not stop workers; preserving module" >&2
    fi
  elif [ "$MODULE_LOADED" -eq 1 ]; then
    printf '%s\n' "raw abort-write-release cleanup incomplete; module intentionally left resident" >&2
  fi
  exit "$status"
}
...
RAW_OUTPUT=$(run_app_command "raw abort write release run $TOKEN")
printf '%s\n' "$RAW_OUTPUT" >> "$EVIDENCE"
require_contains "$RAW_OUTPUT" 'raw mode=abort-write-release failures=0'
require_contains "$RAW_OUTPUT" 'source=raw_va_rx_write'
require_contains "$RAW_OUTPUT" 'action=restore_original'
require_contains "$RAW_OUTPUT" 'logical_release=1'
...
require_contains "$RAW_OUTPUT" 'skip_origin=0'
require_contains "$RAW_OUTPUT" 'normal_value=42'
require_contains "$RAW_OUTPUT" 'shadow_value=99'
require_contains "$RAW_OUTPUT" 'post_release_exec_value=42'
require_contains "$RAW_OUTPUT" 'restored_value=42'
require_contains "$RAW_OUTPUT" 'release_write_word=52800540'
require_contains "$RAW_OUTPUT" 'release_write_completed=0'
require_contains "$RAW_OUTPUT" 'release_write_fault_caught=1'
require_contains "$RAW_OUTPUT" 'handler_faults=1'
require_contains "$RAW_OUTPUT" 'raw_abort_write_release_ready'
require_contains "$RAW_OUTPUT" 'raw_abort_write_release_status'
require_contains "$RAW_OUTPUT" 'armed=0'
...
require_contains "$RAW_OUTPUT" 'restore_result=0'
require_contains "$RAW_OUTPUT" 'active_kind=original'
require_contains "$RAW_OUTPUT" 'record_state=restoring'

EVENT_OUTPUT=$(run_app_command "events $TOKEN")
ACTIVATE_COUNT=$(printf '%s\n' "$EVENT_OUTPUT" | grep -c 'op=21 result=0' || true)
RELEASE_COUNT=$(printf '%s\n' "$EVENT_OUTPUT" | grep -c 'op=40 result=0' || true)
...
[ "$RELEASE_COUNT" -eq 1 ] || fail "expected one raw abort-write-release event, got $RELEASE_COUNT"
...
printf 'raw_abort_write_release=pass warn_after=%s final_modules=empty result=pass\n' \
