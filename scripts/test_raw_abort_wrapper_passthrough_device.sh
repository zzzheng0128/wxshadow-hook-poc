...
IDLE_SECONDS=15
CLEAN_BOOT_CONFIRMED=${RAW_ABORT_PASSTHROUGH_CLEAN_BOOT_CONFIRMED:-}
TOKEN=${RAW_ABORT_PASSTHROUGH_TOKEN:-0x729296}
EVIDENCE_DIR="$ROOT/build/evidence"
EVIDENCE="$EVIDENCE_DIR/raw-abort-wrapper-passthrough-$(date +%Y%m%d-%H%M%S).log"
MODULE_LOADED=0
SESSION_OPEN=0
HOLD_ACTIVE=0

fail() {
  printf '%s\n' "raw abort-wrapper passthrough failure: $*" >&2
  exit 1
}
...
case "$CLEAN_BOOT_CONFIRMED" in
  1) ;;
  *)
    fail "RAW_ABORT_PASSTHROUGH_CLEAN_BOOT_CONFIRMED=1 is required before device access"
    ;;
esac
...
ensure_clean_source() {
  ...
  ) || fail "D4-R3g requires a git clean-source temporary tree"
  [ -z "$dirty" ] ||
    fail "D4-R3g refuses tracked source changes: $dirty"
}
...
preserve_active_hold() {
  ...
  printf '%s\n' \
    "raw abort-wrapper passthrough preserving module: active raw hold remains and cleanup reentry is forbidden" >&2
  exit "$status"
}
...
cleanup() {
  ...
  if [ "$HOLD_ACTIVE" -eq 1 ]; then
    printf 'classification=D4-R3g-setup-blocked result=classified active_hold=unknown cleanup=not_run reason=trap status=%s post_hold_status=not_used boot_id_reader=not_used getprop=not_used proc_maps=not_used pstore=not_used\n' \
      "$status" >> "$EVIDENCE"
    printf '%s\n' \
      "raw abort-wrapper passthrough preserving module: raw hold may be active and cleanup reentry is forbidden" >&2
    exit "$status"
  fi
...
poll_adb_transport() {
  ...
  printf 'phase=d4_r3g_abort_passthrough_idle evidence=begin idle_seconds=%s post_hold_status=not_used boot_id_reader=not_used getprop=not_used proc_maps=not_used pstore=not_used cleanup=preserve\n' \
  ...
  while [ "$elapsed" -lt "$IDLE_SECONDS" ]; do
    ...
    printf 'phase=d4_r3g_abort_passthrough_idle_poll second=%s adb_state_rc=%s adb_state="%s"\n' \
      "$elapsed" "$rc" "$state" >> "$EVIDENCE"
    if [ "$rc" -ne 0 ] || [ "$state" != device ]; then
      printf 'phase=d4_r3g_abort_passthrough_idle result=fail classification=D4-R3g-source-uxn-abort-passthrough-unstable second=%s adb_state_rc=%s adb_state="%s"\n' \
        "$elapsed" "$rc" "$state" >> "$EVIDENCE"
      preserve_active_hold 1 D4-R3g-source-uxn-abort-passthrough-unstable \
        adb-transport-changed
    fi
  done
  printf 'phase=d4_r3g_abort_passthrough_idle result=pass classification=D4-R3g-source-uxn-abort-passthrough-stable idle_seconds=%s post_hold_status=not_used boot_id_reader=not_used getprop=not_used proc_maps=not_used pstore=not_used cleanup=preserve\n' \
...
WARN_BEFORE=$(adb_device shell su -c cat /sys/kernel/warn_count |
  LC_ALL=C tr -d '\r')
printf 'serial=%s lab_uid=%s idle_seconds=%s token=%s warn_before=%s source_tree=clean clean_boot_confirmed=1 diagnostic=global_abort_wrapper_passthrough\n' \
...
HOLD_ACTIVE=1
RAW_OUTPUT=$(run_app_command "raw raw-hold abort-passthrough $TOKEN")
printf '%s\n' "$RAW_OUTPUT" >> "$EVIDENCE"
require_contains "$RAW_OUTPUT" 'raw mode=raw-hold-abort-passthrough'
...
require_contains "$RAW_OUTPUT" 'abort_hook_passthrough=1'
require_contains "$RAW_OUTPUT" 'generation='
printf 'phase=d4_r3g_abort_passthrough_established result=pass target_state=source_uxn raw_slots=1 page_records=1 abort_hook_installed=1 abort_hook_suppressed=0 abort_hook_passthrough=1 post_hold_status=not_used boot_id_reader=not_used getprop=not_used proc_maps=not_used pstore=not_used\n' \
  >> "$EVIDENCE"
