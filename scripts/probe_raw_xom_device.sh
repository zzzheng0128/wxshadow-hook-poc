...
EVIDENCE="$EVIDENCE_DIR/raw-xom-device-$(date +%Y%m%d-%H%M%S).log"
MODULE_LOADED=0
SESSION_OPEN=0

if [ "${R0LAB_RUN_UNSAFE_RAW_XOM:-0}" != "1" ]; then
  mkdir -p "$EVIDENCE_DIR"
  {
    printf '%s\n' 'raw_xom_admission=blocked'
    printf '%s\n' 'result=blocked reason=disabled_after_kernel_panic'
    printf '%s\n' 'last_crash_evidence=build/evidence/pstore-console-raw-xom-20260723-093120.log'
    printf '%s\n' 'last_crash_summary=kernel_panic current_el_dabt stack_overflow_after_raw_xom_ready'
  } | tee "$EVIDENCE"
  printf '%s\n' "$EVIDENCE"
  exit 0
fi
...
cleanup() {
  ...
  if [ "$SESSION_OPEN" -eq 1 ]; then
    ...
    case "$CLEANUP_STATUS" in
      *"raw_slots=0"*) ;;
      *)
        printf '%s\n' "raw-xom cleanup did not confirm an empty raw slot; preserving module" >&2
        cleanup_ok=0
        ;;
    esac
  ...
  if [ "$MODULE_LOADED" -eq 1 ] && [ "$cleanup_ok" -eq 1 ]; then
    if prepare_worker_shutdown >/dev/null 2>&1; then
      ...
      printf '%s\n' "raw-xom cleanup could not stop workers; preserving module" >&2
    fi
  elif [ "$MODULE_LOADED" -eq 1 ]; then
    printf '%s\n' "raw-xom cleanup incomplete; module intentionally left resident" >&2
  fi
  exit "$status"
}
...
require_contains "$RAW_XOM_OUTPUT" 'raw mode=xom-admission failures=0'
...
printf 'raw_xom_admission=%s warn_after=%s final_modules=empty result=pass\n' \
