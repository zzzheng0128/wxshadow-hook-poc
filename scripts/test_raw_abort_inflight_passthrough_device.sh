...
IDLE_SECONDS=15
CLEAN_BOOT_CONFIRMED=${RAW_ABORT_INFLIGHT_CLEAN_BOOT_CONFIRMED:-}
TOKEN=${RAW_ABORT_INFLIGHT_TOKEN:-0x729299}
KEYSTORE=${R0LAB_DEBUG_KEYSTORE:-$ROOT/lab-app/debug.keystore}
EVIDENCE_DIR="$ROOT/build/evidence"
EVIDENCE="$EVIDENCE_DIR/raw-abort-inflight-passthrough-$(date +%Y%m%d-%H%M%S).log"
...
case "$CLEAN_BOOT_CONFIRMED" in
  1) ;;
  *)
    fail "RAW_ABORT_INFLIGHT_CLEAN_BOOT_CONFIRMED=1 is required before device access"
    ;;
esac
...
ensure_clean_source() {
  ...
  ) || fail "D4-R3j requires a git clean-source temporary tree"
  [ -z "$dirty" ] ||
    fail "D4-R3j refuses tracked source changes: $dirty"
}
...
require_tagged_head() {
  ...
  [ "$head" = "$tagged" ] ||
    fail "D4-R3j requires the source-tagged HEAD: tag=$tag tagged=$tagged head=$head"
}
...
preserve_active_hold() {
  ...
  printf '%s\n' \
    "raw abort inflight passthrough preserving module: active raw hold remains and cleanup reentry is forbidden" >&2
  exit "$status"
}
...
cleanup() {
  ...
  if [ "$HOLD_ACTIVE" -eq 1 ]; then
    printf 'classification=D4-R3j-setup-blocked result=classified active_hold=unknown cleanup=not_run reason=trap status=%s post_hold_status=not_used boot_id_reader=not_used getprop=not_used proc_maps=not_used pstore=not_used\n' \
      "$status" >> "$EVIDENCE"
    printf '%s\n' \
      "raw abort inflight passthrough preserving module: raw hold may be active and cleanup reentry is forbidden" >&2
    exit "$status"
  fi
  ...
  if [ "$status" -ne 0 ]; then
    printf 'classification=D4-R3j-setup-blocked result=classified active_hold=0 cleanup=bounded reason=trap status=%s post_hold_status=not_applicable boot_id_reader=not_used getprop=not_used proc_maps=not_used pstore=not_used\n' \
      "$status" >> "$EVIDENCE"
  fi
  exit "$status"
}
...
poll_adb_transport() {
  ...
  printf 'phase=d4_r3j_abort_inflight_idle evidence=begin idle_seconds=%s post_hold_status=not_used boot_id_reader=not_used getprop=not_used proc_maps=not_used pstore=not_used cleanup=preserve\n' \
  ...
  while [ "$elapsed" -lt "$IDLE_SECONDS" ]; do
    ...
    printf 'phase=d4_r3j_abort_inflight_idle_poll second=%s adb_state_rc=%s adb_state="%s"\n' \
      "$elapsed" "$rc" "$state" >> "$EVIDENCE"
    if [ "$rc" -ne 0 ] || [ "$state" != device ]; then
      printf 'phase=d4_r3j_abort_inflight_idle result=fail classification=D4-R3j-source-uxn-abort-inflight-unstable second=%s adb_state_rc=%s adb_state="%s"\n' \
        "$elapsed" "$rc" "$state" >> "$EVIDENCE"
      preserve_active_hold 1 D4-R3j-source-uxn-abort-inflight-unstable \
        adb-transport-changed
    fi
  done
  printf 'phase=d4_r3j_abort_inflight_idle result=pass classification=D4-R3j-source-uxn-abort-inflight-stable idle_seconds=%s post_hold_status=not_used boot_id_reader=not_used getprop=not_used proc_maps=not_used pstore=not_used cleanup=preserve\n' \
    "$IDLE_SECONDS" | tee -a "$EVIDENCE"
  preserve_active_hold 0 D4-R3j-source-uxn-abort-inflight-stable \
    idle-window-complete
}

ensure_clean_source
require_tagged_head wxshadow-v2-f46-d4-r3j-inflight-accounting-source-20260724
...
WARN_BEFORE=$(adb_device shell su -c cat /sys/kernel/warn_count |
  LC_ALL=C tr -d '\r')
printf 'serial=%s lab_uid=%s idle_seconds=%s token=%s warn_before=%s source_tree=clean clean_boot_confirmed=1 diagnostic=global_abort_r0lab_inflight kpm_sha256=%s labprobe_so_sha256=%s classes_dex_sha256=%s signer_cert_sha256=%s\n' \
...
printf 'phase=d4_r3j_abort_inflight_established result=pass target_state=source_uxn raw_slots=1 page_records=1 abort_hook_installed=1 abort_hook_suppressed=0 abort_hook_passthrough=0 abort_hook_mmget=0 abort_hook_lock=0 abort_hook_inflight=1 post_hold_status=not_used boot_id_reader=not_used getprop=not_used proc_maps=not_used pstore=not_used\n' \
  >> "$EVIDENCE"
