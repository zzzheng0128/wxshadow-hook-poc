...
TOKEN=${RAW_ABORT_PROBE_TOKEN:-0x729239}
EVIDENCE_DIR="$ROOT/build/evidence"
EVIDENCE="$EVIDENCE_DIR/raw-abort-probe-$(date +%Y%m%d-%H%M%S).log"
MODULE_LOADED=0
SESSION_OPEN=0
...
cleanup() {
  ...
  if [ "$SESSION_OPEN" -eq 1 ]; then
    ...
    case "$CLEANUP_STATUS" in
      *"raw_slots=0"*) ;;
      *)
        printf '%s\n' "raw abort-probe cleanup did not confirm an empty raw slot; preserving module" >&2
        cleanup_ok=0
        ;;
    esac
  ...
  if [ "$MODULE_LOADED" -eq 1 ] && [ "$cleanup_ok" -eq 1 ]; then
    if prepare_worker_shutdown >/dev/null 2>&1; then
      ...
      printf '%s\n' "raw abort-probe cleanup could not stop workers; preserving module" >&2
    fi
  elif [ "$MODULE_LOADED" -eq 1 ]; then
    printf '%s\n' "raw abort-probe cleanup incomplete; module intentionally left resident" >&2
  fi
  exit "$status"
}
...
RAW_OUTPUT=$(run_app_command "raw abort probe run $TOKEN")
printf '%s\n' "$RAW_OUTPUT" >> "$EVIDENCE"
require_contains "$RAW_OUTPUT" 'raw mode=abort-probe failures=0'
...
require_contains "$RAW_OUTPUT" 'probe_read_word=52800c60'
require_contains "$RAW_OUTPUT" 'probe_read_completed=1'
...
require_contains "$RAW_OUTPUT" 'permission_fault=0'
...
[ "$ABORT_PROBE_COUNT" -eq 1 ] || fail "expected one raw abort-probe event, got $ABORT_PROBE_COUNT"
...
printf 'raw_abort_probe=pass warn_after=%s final_modules=empty result=pass\n' \
