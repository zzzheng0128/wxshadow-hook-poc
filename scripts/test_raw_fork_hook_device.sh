...
TOKEN=${RAW_FORK_HOOK_TOKEN:-0x729202}
EVIDENCE_DIR="$ROOT/build/evidence"
EVIDENCE="$EVIDENCE_DIR/raw-fork-hook-$(date +%Y%m%d-%H%M%S).log"
MODULE_LOADED=0
SESSION_OPEN=0
...
cleanup() {
  ...
  if [ "$SESSION_OPEN" -eq 1 ]; then
    run_app_command "raw fork hook clear $TOKEN" >/dev/null 2>&1 || true
    run_app_command "raw clear $TOKEN" >/dev/null 2>&1 || true
    CLEANUP_STATUS=$(run_app_command status 2>&1 || true)
    case "$CLEANUP_STATUS" in
      *"raw_slots=0"*) ;;
      *)
        printf '%s\n' "raw fork-hook cleanup did not confirm an empty raw slot; preserving module" >&2
        cleanup_ok=0
        ;;
    esac
  ...
  if [ "$MODULE_LOADED" -eq 1 ] && [ "$cleanup_ok" -eq 1 ]; then
    if prepare_worker_shutdown >/dev/null 2>&1; then
      ...
      printf '%s\n' "raw fork-hook cleanup could not stop workers; preserving module" >&2
    fi
  elif [ "$MODULE_LOADED" -eq 1 ]; then
    printf '%s\n' "raw fork-hook cleanup incomplete; module intentionally left resident" >&2
  fi
  exit "$status"
}
...
RAW_OUTPUT=$(run_app_command "raw fork hook run $TOKEN")
printf '%s\n' "$RAW_OUTPUT" >> "$EVIDENCE"
require_contains "$RAW_OUTPUT" 'raw mode=fork-hook failures=0'
...
require_contains "$RAW_OUTPUT" 'child_word=52800540'
require_contains "$RAW_OUTPUT" 'child_value=42'
require_contains "$RAW_OUTPUT" 'shadow_after_fork=52800c60'
require_contains "$RAW_OUTPUT" 'shadow_after_fork_value=99'
require_contains "$RAW_OUTPUT" 'restored_value=42'
require_contains "$RAW_OUTPUT" 'child_result_read=12'
require_contains "$RAW_OUTPUT" 'child_errno=0'
require_contains "$RAW_OUTPUT" 'child_exit=0'
require_contains "$RAW_OUTPUT" 'raw_fork_hook_ready'
...
require_contains "$RAW_OUTPUT" 'fork_hide_primitive=proven'
...
[ "$FORK_HOOK_BEGIN_COUNT" -eq 1 ] || fail "expected one raw fork hook begin event, got $FORK_HOOK_BEGIN_COUNT"
[ "$FORK_HOOK_FINISH_COUNT" -eq 1 ] || fail "expected one raw fork hook finish event, got $FORK_HOOK_FINISH_COUNT"
...
printf 'raw_fork_hook=pass warn_after=%s final_modules=empty result=pass\n' \
