...
IDLE_SECONDS=15
CLEAN_BOOT_CONFIRMED=${RAW_ABORT_IABT_TRANSITION_CLEAN_BOOT_CONFIRMED:-}
TOKEN=${RAW_ABORT_IABT_TRANSITION_TOKEN:-0x72939a}
...
fail() {
  printf '%s\n' "raw abort IABT transition failure: $*" >&2
  exit 1
}
...
case "$CLEAN_BOOT_CONFIRMED" in
  1) ;;
  *)
    fail "RAW_ABORT_IABT_TRANSITION_CLEAN_BOOT_CONFIRMED=1 is required before device access"
    ;;
esac
...
ensure_clean_source() {
  ...
  ) || fail "D4-R3m requires a git clean-source temporary tree"
  [ -z "$dirty" ] ||
    fail "D4-R3m refuses tracked source changes: $dirty"
}
...
require_tagged_head() {
  ...
  [ "$head" = "$tagged" ] ||
    fail "D4-R3m requires the source-tagged HEAD: tag=$tag tagged=$tagged head=$head"
  SOURCE_COMMIT=$head
  SOURCE_TAG=$tag
}
...
preserve_active_hold() {
  ...
  printf '%s\n' \
    "raw abort IABT transition preserving module: active raw hold remains and cleanup reentry is forbidden" >&2
  exit "$status"
}
...
require_active_contains() {
  ...
  needle=$2
  printf '%s\n' "$haystack" | LC_ALL=C grep -F -- "$needle" >/dev/null ||
    preserve_active_hold 1 D4-R3m-source-uxn-iabt-transition-unstable \
...
cleanup() {
  ...
  if [ "$HOLD_ACTIVE" -eq 1 ]; then
    printf 'classification=D4-R3m-setup-blocked result=classified active_hold=unknown cleanup=not_run reason=trap status=%s post_hold_status=not_used boot_id_reader=not_used getprop=not_used proc_maps=not_used pstore=not_used\n' \
      "$status" >> "$EVIDENCE"
    printf '%s\n' \
      "raw abort IABT transition preserving module: raw hold may be active and cleanup reentry is forbidden" >&2
    exit "$status"
  fi
  ...
  if [ "$status" -ne 0 ]; then
    printf 'classification=D4-R3m-setup-blocked result=classified active_hold=0 cleanup=bounded reason=trap status=%s post_hold_status=not_applicable boot_id_reader=not_used getprop=not_used proc_maps=not_used pstore=not_used\n' \
      "$status" >> "$EVIDENCE"
  fi
  exit "$status"
}
...
poll_adb_transport() {
  ...
  printf 'phase=d4_r3m_abort_iabt_transition_idle evidence=begin idle_seconds=%s post_hold_status=not_used boot_id_reader=not_used getprop=not_used proc_maps=not_used pstore=not_used cleanup=preserve\n' \
  ...
  while [ "$elapsed" -lt "$IDLE_SECONDS" ]; do
    ...
    printf 'phase=d4_r3m_abort_iabt_transition_idle_poll second=%s adb_state_rc=%s adb_state="%s"\n' \
      "$elapsed" "$rc" "$state" >> "$EVIDENCE"
    if [ "$rc" -ne 0 ] || [ "$state" != device ]; then
      printf 'phase=d4_r3m_abort_iabt_transition_idle result=fail classification=D4-R3m-source-uxn-iabt-transition-unstable second=%s adb_state_rc=%s adb_state="%s"\n' \
        "$elapsed" "$rc" "$state" >> "$EVIDENCE"
      preserve_active_hold 1 D4-R3m-source-uxn-iabt-transition-unstable \
        adb-transport-changed
    fi
  done
  printf 'phase=d4_r3m_abort_iabt_transition_idle result=pass classification=D4-R3m-source-uxn-iabt-transition-stable idle_seconds=%s post_hold_status=not_used boot_id_reader=not_used getprop=not_used proc_maps=not_used pstore=not_used cleanup=preserve\n' \
    "$IDLE_SECONDS" | tee -a "$EVIDENCE"
  preserve_active_hold 0 D4-R3m-source-uxn-iabt-transition-stable \
...
"$ROOT/scripts/verify_d4_r3m_disassembly.sh" >/dev/null
...
PRE_HOLD_BOOT_ID=$(adb_device shell cat /proc/sys/kernel/random/boot_id |
  LC_ALL=C tr -d '\r') ||
  fail "pre-hold boot ID read failed"
[ "$PRE_HOLD_BOOT_ID" = "$EXPECTED_PRE_HOLD_BOOT_ID" ] ||
  fail "unexpected pre-hold boot ID: expected=$EXPECTED_PRE_HOLD_BOOT_ID actual=$PRE_HOLD_BOOT_ID"
...
WARN_BEFORE=$(adb_device shell su -c cat /sys/kernel/warn_count |
  LC_ALL=C tr -d '\r')
printf 'serial=%s lab_uid=%s idle_seconds=%s token=%s warn_before=%s pre_hold_boot_id=%s source_tree=clean clean_boot_confirmed=1 diagnostic=global_abort_r0lab_iabt_transition_restore_abi source_commit=%s source_tag=%s kpm_sha256=%s labprobe_so_sha256=%s classes_dex_sha256=%s signer_cert_sha256=%s\n' \
  "${SERIAL:-default}" "$LAB_UID" "$IDLE_SECONDS" "$TOKEN" "$WARN_BEFORE" \
  "$PRE_HOLD_BOOT_ID" "$SOURCE_COMMIT" "$SOURCE_TAG" "$KPM_SHA" "$LABPROBE_SHA" \
...
HOLD_ACTIVE=1
RAW_OUTPUT=$(run_app_command "raw raw-hold abort-iabt-transition $TOKEN")
printf '%s\n' "$RAW_OUTPUT" >> "$EVIDENCE"
require_active_contains "$RAW_OUTPUT" 'raw mode=raw-hold-abort-iabt-transition'
require_active_contains "$RAW_OUTPUT" 'failures=0'
require_active_contains "$RAW_OUTPUT" 'exit_mmap_armed=0'
require_active_contains "$RAW_OUTPUT" 'target_state=shadow_rx'
require_active_contains "$RAW_OUTPUT" 'slots=1'
require_active_contains "$RAW_OUTPUT" 'raw_slots=1'
require_active_contains "$RAW_OUTPUT" 'page_records=1'
require_active_contains "$RAW_OUTPUT" 'normal=42'
require_active_contains "$RAW_OUTPUT" 'shadow=99'
require_active_contains "$RAW_OUTPUT" 'activations=1'
require_active_contains "$RAW_OUTPUT" 'state=3'
require_active_contains "$RAW_OUTPUT" 'active_kind=shadow_rx'
require_active_contains "$RAW_OUTPUT" 'record_state=shadow_active'
require_active_contains "$RAW_OUTPUT" 'inspect=1'
require_active_contains "$RAW_OUTPUT" 'abort_hook_installed=1'
require_active_contains "$RAW_OUTPUT" 'abort_hook_suppressed=0'
require_active_contains "$RAW_OUTPUT" 'abort_hook_passthrough=0'
require_active_contains "$RAW_OUTPUT" 'abort_hook_mmget=0'
require_active_contains "$RAW_OUTPUT" 'abort_hook_lock=0'
require_active_contains "$RAW_OUTPUT" 'abort_hook_inflight=0'
require_active_contains "$RAW_OUTPUT" 'abort_hook_iabt_route=0'
require_active_contains "$RAW_OUTPUT" 'abort_hook_iabt_transition=1'
require_active_contains "$RAW_OUTPUT" 'handler_faults=0'
require_active_contains "$RAW_OUTPUT" 'signal_fault_caught=0'
require_active_contains "$RAW_OUTPUT" 'restore_abi=skip_origin_ret0'
require_active_contains "$RAW_OUTPUT" 'generation='
printf 'phase=d4_r3m_abort_iabt_transition_established result=pass target_state=shadow_rx raw_slots=1 page_records=1 normal=42 shadow=99 activations=1 state=3 active_kind=shadow_rx record_state=shadow_active abort_hook_installed=1 abort_hook_suppressed=0 abort_hook_passthrough=0 abort_hook_mmget=0 abort_hook_lock=0 abort_hook_inflight=0 abort_hook_iabt_route=0 abort_hook_iabt_transition=1 handler_faults=0 signal_fault_caught=0 restore_abi=skip_origin_ret0 post_hold_status=not_used boot_id_reader=not_used getprop=not_used proc_maps=not_used pstore=not_used\n' \
  >> "$EVIDENCE"
