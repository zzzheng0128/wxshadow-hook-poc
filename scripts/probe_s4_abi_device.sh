...
EVIDENCE="$EVIDENCE_DIR/s4-abi-$(date +%Y%m%d-%H%M%S).log"
MODULE_LOADED=0
...
ABI_OUTPUT=$(run_app_command 's4 abi')
require_contains "$ABI_OUTPUT" 's4_abi ready='
require_contains "$ABI_OUTPUT" 'brk_handler='
require_contains "$ABI_OUTPUT" 'single_step_handler='
require_contains "$ABI_OUTPUT" 'user_enable_single_step='
require_contains "$ABI_OUTPUT" 'user_disable_single_step='
require_contains "$ABI_OUTPUT" 'mode=probe_only'
printf '%s\n' "$ABI_OUTPUT" >> "$EVIDENCE"
S4_READY=$(printf '%s\n' "$ABI_OUTPUT" |
  sed -n 's/.*s4_abi ready=\([^ ]*\).*/\1/p' | sed -n '1p')
[ -n "$S4_READY" ] || fail "missing S4 ready field"
...
printf 's4_ready=%s warn_after=%s final_modules=empty result=pass\n' \
  "$S4_READY" "$WARN_AFTER" | tee -a "$EVIDENCE"
printf '%s\n' "$EVIDENCE"
