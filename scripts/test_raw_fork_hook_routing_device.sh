...
TOKEN=${RAW_FORK_HOOK_ROUTING_TOKEN:-0x729222}
EVIDENCE_DIR="$ROOT/build/evidence"
EVIDENCE="$EVIDENCE_DIR/raw-fork-hook-routing-$(date +%Y%m%d-%H%M%S).log"
MODULE_LOADED=0
SESSION_OPEN=0
...
fail() {
  printf '%s\n' "raw fork-hook-routing failure: $*" >&2
  exit 1
}
...
cleanup() {
  ...
  if [ "$SESSION_OPEN" -eq 1 ]; then
    run_app_command "raw slot fork hook clear $TOKEN 0" >/dev/null 2>&1 || true
    run_app_command "raw slot fork hook clear $TOKEN 1" >/dev/null 2>&1 || true
    ...
    case "$CLEANUP_STATUS" in
      *"raw_slots=0"*) ;;
      *)
        printf '%s\n' "raw fork-hook-routing cleanup did not confirm empty raw slots; preserving module" >&2
        cleanup_ok=0
        ;;
    esac
  ...
  if [ "$MODULE_LOADED" -eq 1 ] && [ "$cleanup_ok" -eq 1 ]; then
    if prepare_worker_shutdown >/dev/null 2>&1; then
      ...
      printf '%s\n' "raw fork-hook-routing cleanup could not stop workers; preserving module" >&2
    fi
  elif [ "$MODULE_LOADED" -eq 1 ]; then
    printf '%s\n' "raw fork-hook-routing cleanup incomplete; module intentionally left resident" >&2
  fi
  exit "$status"
}
...
RAW_OUTPUT=$(run_app_command "raw fork hook routing run $TOKEN")
printf '%s\n' "$RAW_OUTPUT" >> "$EVIDENCE"
require_contains "$RAW_OUTPUT" 'raw mode=fork-hook-routing failures=0'
require_contains "$RAW_OUTPUT" 'parent_pause=1'
require_contains "$RAW_OUTPUT" 'child_original_inherit=1'
require_contains "$RAW_OUTPUT" 'target_mm_scoped=1'
require_contains "$RAW_OUTPUT" 'page_record_routed=1'
require_contains "$RAW_OUTPUT" 'status_source=cross_clear'
require_contains "$RAW_OUTPUT" 'normal=42/42 shadow=99/99'
require_contains "$RAW_OUTPUT" 'child_word=52800540/52800540'
require_contains "$RAW_OUTPUT" 'child_value=42/42'
require_contains "$RAW_OUTPUT" 'after_fork_shadow=52800c60/52800c60'
require_contains "$RAW_OUTPUT" 'after_fork_shadow_values=99/99'
...
require_contains "$RAW_OUTPUT" 'result_read=20'
require_contains "$RAW_OUTPUT" 'child_errno=0'
require_contains "$RAW_OUTPUT" 'handler_faults=0'
require_contains "$RAW_OUTPUT" 'raw_slot_fork_hook_ready slot=0'
require_contains "$RAW_OUTPUT" 'raw_slot_fork_hook_ready slot=1'
...
require_contains "$RAW_OUTPUT" 'hook0_clear="raw_slot_fork_hook_cleared slot=0'
require_contains "$RAW_OUTPUT" 'hook1_clear="raw_slot_fork_hook_cleared slot=1'

EVENT_OUTPUT=$(run_app_command "events $TOKEN")
ACTIVATE_COUNT=$(printf '%s\n' "$EVENT_OUTPUT" | grep -c 'op=21 result=0' || true)
FORK_HOOK_BEGIN_COUNT=$(printf '%s\n' "$EVENT_OUTPUT" | grep -c 'op=31 result=0' || true)
FORK_HOOK_FINISH_COUNT=$(printf '%s\n' "$EVENT_OUTPUT" | grep -c 'op=32 result=0' || true)
[ "$ACTIVATE_COUNT" -eq 2 ] || fail "expected two raw activation events, got $ACTIVATE_COUNT"
[ "$FORK_HOOK_BEGIN_COUNT" -eq 2 ] || fail "expected two raw fork hook begin events, got $FORK_HOOK_BEGIN_COUNT"
[ "$FORK_HOOK_FINISH_COUNT" -eq 2 ] || fail "expected two raw fork hook finish events, got $FORK_HOOK_FINISH_COUNT"
...
printf 'raw_fork_hook_routing=pass route=fork page_record_routed=1 warn_after=%s final_modules=empty result=pass\n' \
