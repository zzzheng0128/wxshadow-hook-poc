...
require_text lab-app/src/main/cpp/labprobe.c 'exit_hook_installed=%d/%d'
...
require_text "$F46_PANIC_DIAGNOSIS" 'D4-R3b is currently a plan-lock packet'
...
require_text "$F46_PANIC_DIAGNOSIS" 'no D4-R3b device evidence has been collected yet'
...
require_text "$F46_PANIC_DIAGNOSIS" 'Do not rerun or patch D4 before D4-R3 converts'
...
require_text "$DEVELOPMENT_SEQUENCE" 'F4.6-D4-R3b Lab App-only raw-hold split | Implemented locally; device pending'
...
require_text "$F46_D4_R3B_PLAN" 'Device verification is not yet run'
...
require_text scripts/test_raw_exit_hook_raw_hold_split_device.sh 'exit_hook_installed=0/0'
...
require_text "$FINAL_ROADMAP" 'checkpoint is D4-R3b-L local implementation'
...
require_text "$FINAL_ROADMAP" 'reboot-risking device reader ladder remains pending'
