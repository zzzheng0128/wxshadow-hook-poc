...
require_text "$F46_PANIC_DIAGNOSIS" 'Do not patch D4 before D4-R3d converts'
...
require_text "$DEVELOPMENT_SEQUENCE" 'F4.6-D4-R3d status-transport split | Planning'
...
require_text "$DEVELOPMENT_SEQUENCE" 'diagnostic gate is F4.6-D4-R3d'
...
require_text "$F46_D4_R3D_PLAN" 'Status: plan-only gate'
...
require_text "$DEVELOPMENT_SEQUENCE" 'D4-R3c-status-logcat-timeout-kernel-panic'
require_text "$DEVELOPMENT_SEQUENCE" 'raw-hold idle stability, Activity/Logcat status transport, KPM status supercall behavior, and maps/seq_file reader behavior'
